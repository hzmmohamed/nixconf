{self, ...}: {
  ...
  in {
    ...
    home-manager.users.${user} = {
      # Custom systemd target that signals the compositor env vars (DISPLAY,
      # WAYLAND_DISPLAY, XDG_CURRENT_DESKTOP) have been imported into the user
      # session. Started by sway's exec after import-environment completes.
      # Services needing these vars should use After/Requires on this target.
      systemd.user.targets.sway-session-env-ready = {
        Unit = {
          Description = "Sway session environment variables are ready";
      ...
      wayland.windowManager.sway = {
        ...
        config = {
          ...
          startup =
            [
              {
                command = "systemctl --user import-environment DISPLAY WAYLAND_DISPLAY XDG_CURRENT_DESKTOP && dbus-update-activation-environment --systemd DISPLAY WAYLAND_DISPLAY XDG_CURRENT_DESKTOP && systemctl --user start sway-session-env-ready.target";
                always = true;
              }
            ]
            ++ map (entry: {
